...
export function getAllSummary(callback) {
    ...
    BPromise.promisify(db.getLogTable)().then(function(rows) {
        ...
        BPromise.promisify(db.getUserTable)().then(function(rows) {
            numRegisteredUsers = rows.length;
        }).then(function() {
            ...
            var mostFrequentCommands = _.chain(commandsLastWeek).countBy().pairs().sortBy(function(pair) {
                return pair[1];
            }).reverse().value();
